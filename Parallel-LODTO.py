...
def Pdly(s, n): #Time threshold probability
    return np.exp(-Tth * ((Miun(Beta[s][s], s, n) / Tslt) - Lambdan))


def rho(s, n):
    return Lambdan * Tslt / (Miun(Beta[s][s], s, n))
...
def EI(m, s, n):
    ...
    for i in range(len(m)):
        sum += Pt * (hn(m[i], n) ** 2) * (
            (quad(lambda x: EIfunc(x, RorR(m[i], n), b(m[i], n)), Beta[s][m[i]], 10))[
                0]) * (Miun(Beta[s][m[i]], s, n) / F)
    return sum
...
def DI(m, s, n):
    ...
    for i in range(len(m)):
        j = 0
        sum += (Pt ** 2) * (hn(m[i], n) ** 4) * (
            (quad(lambda x: DIfunc(x, RorR(m[i], n), b(m[i], n)), Beta[s][m[i]], 10))[
                0]) * ((Miun(Beta[s][m[i]], s, n) / F) ** 2)
        while j < i:
            sum1 += 2 * Pt * (hn(m[i], n) ** 2) * (
                (quad(lambda x: EIfunc(x, RorR(m[i], n), b(m[i], n)), Beta[s][m[i]], 10))[
                    0]) * (
                            Miun(Beta[s][m[j]], s, n) / F) * Pt * (hn(m[j], n) ** 2) * (
                        (quad(lambda x: EIfunc(x, RorR(m[j], n), b(m[j], n)), Beta[s][m[j]], 10))[0]) * (
                            Miun(Beta[s][m[j]], s, n) / F)
            j += 1
...
def Perr(m, s, n): #Transmission error probability
    return quad(lambda x: Perrfunc(x, m, s, n), Beta[s][s], 10)[0]
...
def parallel_LODTO(j, mp, psi, prv_Betan):
    m_local = [x for x in mp if x not in [mp[j], size - (mp[j] + 1)]]
    for k in range(len(mp)):
        if random.random() >= psi or j == k:
            Beta[mp[j]][mp[k]] = prv_Betan[mp[k]]
        else:
            print(f"Node {mp[k]} Dropped for Beta[{mp[j]}][{mp[k]}]")
    print(f'Beta[{mp[j]}]:', Beta[mp[j]])
    bst_rn = Rn(m_local, mp[j], size - (mp[j] + 1))
    print('Interferers:', m_local, 'Transmitter:', mp[j], 'Receiver:', size - (mp[j] + 1), 'Throughput:', bst_rn)
    beta_star_j, rn_bst_j = LCS(Beta[mp[j]][mp[j]], bst_rn, j, m_local)
    return mp[j], beta_star_j, rn_bst_j, Beta[mp[j]]


def LODTO(mp, psi=0.5, itr=100):
    global Beta
    betan_star = [BetamUpRician(i) - 0.01 if RorR(i, size - (i + 1)) == 1 else BetaUpRayleigh(i) - 0.01 for i in range(size)]
    Beta = [betan_star.copy() for _ in range(len(mp))]
    rn_bst = betan_star.copy()
    print('Beta:', betan_star)
    for i in range(itr):
        prv_Betan = betan_star.copy()
        with concurrent.futures.ProcessPoolExecutor() as executor:
            futures = [executor.submit(parallel_LODTO, j, mp, psi, prv_Betan) for j in range(len(mp))]
            for future in concurrent.futures.as_completed(futures):
                idx, beta_star_j, rn_bst_j, beta_j = future.result()
                Beta[idx] = beta_j
