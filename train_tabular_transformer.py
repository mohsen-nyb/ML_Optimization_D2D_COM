...
WEIGHT_DECAY = 0
SAVE_CSV = True
OUT_CSV = f"results_tabular/deep_mlp_seed_results_avg_seeds_{num_nodes}_nodes.csv"
DROPOUT = 0.4
...
# ---------- Transformer Model for Tabular Regression ----------
class TabularTransformer(nn.Module):
    def __init__(self, input_dim, output_dim, embed_dim=64, num_heads=4, depth=2, ff_dim=128, dropout=0.1):
        super().__init__()
        self.input_dim = input_dim
        self.embed_dim = embed_dim

        # Linear projection of features to embedding
    ...
    def forward(self, x):
        ...
        x = self.input_proj(x).unsqueeze(1)  # shape -> (batch, seq=1, embed_dim)
        x = self.encoder(x)  # shape -> (batch, seq=1, embed_dim)
        x = x.squeeze(1)  # shape -> (batch, embed_dim)
        out = self.output_head(x)  # shape -> (batch, output_dim)
        return out
...
def train_model(model, train_loader, val_loader, criterion, optimizer, n_epochs=200, patience=20):
...
for seed in SEEDS:
    ...
    g_val = torch.Generator().manual_seed(seed+7)
    g_test = torch.Generator().manual_seed(seed+13)

    train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True, generator=g_train)
    val_loader = DataLoader(val_dataset, batch_size=BATCH_SIZE, shuffle=False, generator=g_val)
    test_loader = DataLoader(test_dataset, batch_size=BATCH_SIZE, shuffle=False, generator=g_test)

    input_dim = X_train_scaled.shape[1]
    output_dim = y_train_scaled.shape[1]

    model = TabularTransformer(input_dim=input_dim, output_dim=output_dim, embed_dim=64, num_heads=4, depth=2, ff_dim=128, dropout=0.1)
    set_all_seeds(seed+1)
    criterion = nn.MSELoss()
    optimizer = optim.Adam(model.parameters(), lr=1e-3)

    t0 = time.time()
    model_trained = train_model(model, train_loader, val_loader, criterion, optimizer, n_epochs=200, patience=20)
    t1 = time.time()
    print(f"Seed {seed} training time: {t1-t0:.1f}s")

    # Predictions (inverse scaling)
    y_train_pred_scaled, y_train_true_scaled = evaluate_model(model_trained, train_loader)
    y_val_pred_scaled, y_val_true_scaled = evaluate_model(model_trained, val_loader)
    y_test_pred_scaled, y_test_true_scaled = evaluate_model(model_trained, test_loader)
    ...
    # Metrics
    for ds_name, y_true, y_pred in [('train', y_train_true, y_train_pred),
                                    ('val', y_val_true, y_val_pred),
                                    ('test', y_test_true, y_test_pred)]:
        metrics = compute_metrics_np(y_true, y_pred)
        row = {'seed': seed, 'model': 'TabularTransformer', 'dataset': ds_name, **metrics}
        all_seed_rows.append(row)
        print(f"Seed {seed} {ds_name.upper()}: R2={metrics['r2']:.4f}, RMSE={metrics['rmse']:.4f}")
...
for ds in ['train','val','test']:
    ...
    summary = {
        ...
        'dataset': ds,
        'mse_mean': sub['mse'].mean(), 'mse_std': sub['mse'].std(),
...
# Save CSV
os.makedirs("results_tabular_transformer", exist_ok=True)
summary_df.to_csv(f"results_tabular_transformer/tabular_transformer_summary_{num_nodes}_nodes.csv", index=False)
