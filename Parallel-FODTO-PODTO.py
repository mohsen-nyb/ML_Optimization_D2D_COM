...
def parallel_FODTO(j, mp):
    ...
    beta_star_j, rn_bst_j = LCS(Beta[mp[j]], bst_rn, j, m_local)
...
def FODTO(mp, itr=100):
    ...
    for i in range(itr):
        ...
        with concurrent.futures.ProcessPoolExecutor() as executor:
            futures = [executor.submit(parallel_FODTO, j, mp) for j in range(len(mp))]
...
def parallel_PODTO(j, mp, psi):
    if random.random() < psi:
        print('Dropped Node:', mp[j])
        return mp[j], None, None, False
    m_local = [x for x in mp if x not in [mp[j], size - (mp[j] + 1)]]
    bst_rn = Rn(m_local, mp[j], size - (mp[j] + 1))
    print('Interferers:', m_local, 'Transmitter:', mp[j], 'Receiver:', size - (mp[j] + 1), 'Throughput:', bst_rn)
    beta_star_j, rn_bst_j = LCS(Beta[mp[j]], bst_rn, j, m_local)
    return mp[j], beta_star_j, rn_bst_j, True
...
def PODTO(mp, psi=0.5, itr=100):
    ...
    for i in range(itr):
        ...
        Beta = prv_Betan.copy()
        with concurrent.futures.ProcessPoolExecutor() as executor:
            futures = [executor.submit(parallel_PODTO, j, mp, psi) for j in range(len(mp))]
            for future in concurrent.futures.as_completed(futures):
                idx, beta_star_j, rn_bst_j, updated_flag = future.result()
                if updated_flag:
                    betan_star[idx] = beta_star_j
                    rn_bst[idx] = rn_bst_j
                    updated_this_iter[idx] = True
                else:
                    updated_this_iter[idx] = False
        print("Optimal Transmission Threshold:", betan_star)
        print("Optimal Throughput:", rn_bst)
        print("Number of Iterations:", i + 1)
        no_inactive = not any(
            (not updated_last_iter[n]) and (not updated_this_iter[n])
            for n in range(len(mp))
        )
        if (betan_star == prv_Betan) and no_inactive:
            break
        updated_last_iter = updated_this_iter.copy()
    return betan_star, rn_bst


print('FODTO:', FODTO(mp))